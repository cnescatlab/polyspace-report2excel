...
def main():
    ...
    if misra or runtime:
        to_export = dict()
        if misra:
            isolate_chapter_misra(tables, to_export)
        elif runtime:
            isolate_chapter_runtime(tables, to_export)
        tables = to_export
...
def summary_sheet(misra, runtime, output):
    if misra or runtime:
